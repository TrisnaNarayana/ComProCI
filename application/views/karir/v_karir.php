        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
								<button class="btn btn-danger btn-sm pull-left" onclick="addKarir()"><i class="fa fa-plus"></i> | Tambah Karir</button>
...
<script>
    ...
    $(document).ready(function () {
      table =  $('#tabel').DataTable({
                    ...
                    "ajax": {
                        "url": "<?php echo base_url().'Karir/ajax_list'?>",
                        "type": "POST"
                    },
...
<script type="text/javascript">
      ...
      $('#dropify').dropify({
                messages: {
                      default: 'Choose file PDF/DOC/DOCX max(2 MB)',
                      replace: 'Update',
                      remove:  'Remove',
                      error:   'error'
                  }
      ...
      $('#formJ').on('submit', function(e){
        ...
        function addKarir()
        {
            ...
            $('label').hide();
            $('#addLomba').modal('show');
            $('.modal-title').text('Tambah Karir');
        }

        function updateKarir(id)
        {
            ...
            $.ajax({
                url : "<?=base_url().'Karir/get/'?>"+id,
                type : "GET",
                dataType : "JSON",
                success : function(data)
                {
										$('[name="id"]').val(data.id_karir);
										$('[name="nama_karir"]').val(data.nama_karir);
                    $('.modal-title').text('Edit Karir');
                    $('#addLomba').modal('show');
                },
